...
function PatientBadge({ statut }: { statut: PatientStatut }) {
  const map: Record<PatientStatut, { bg: string; color: string }> = {
...
function SyncBadge({ pending }: { pending: boolean }) {
  return (
    <span style={{
      background: pending ? "#FFEDD5" : "#D1FAE5",
      color: pending ? "#9A3412" : "#065F46",
      padding: "2px 10px", borderRadius: 12, fontSize: 12, fontWeight: 600,
    }}>
      {pending ? "En attente" : "Synchronisé"}
    </span>
  );
}

function SyncIcon({ type }: { type: SyncType }) {
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        minWidth: 440, maxWidth: 520, width: "100%",
...
export default function AgentPage() {
  ...
  const pendingCount = syncQueue.filter((i) => i.pending).length;
  ...
  function handleAddRdv() {
    ...
    const doctor = doctors.find((d) => d.id === newRdvForm.doctorId);
    addAppointment({
      id: `RDV-${Date.now()}`,
      patientName: newRdvForm.patientName,
      patientPhone: newRdvForm.phone,
  ...
  function handleSyncAll() {
    syncQueue.filter((i) => i.pending).forEach((i) => toggleSyncItem(i.id));
    showToast("Tout synchronisé !");
  }
        ...
        <section>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: C.text }}>Mes patients</h2>
                    ...
                    <td style={{ padding: "11px 14px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button
                          ...
                          style={{
                            background: hovBtn === `rdv-${p.id}` ? C.tealHov : C.teal,
        ...
        <section>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {pendingCount > 0 && (
                <span style={{ background: C.orange, color: "#fff", borderRadius: 12, padding: "1px 10px", fontSize: 12, fontWeight: 700 }}>
          ...
          <div style={{ background: C.card, borderRadius: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.08)", overflow: "hidden" }}>
            {syncQueue.map((item: SyncQueueItem, i: number) => (
              <div
                key={item.id}
                style={{
              ...
              >
                ...
                <SyncBadge pending={item.pending} />
                {item.pending && (
                  <button
                    ...
                    style={{
                      background: hovBtn === `sync-${item.id}` ? C.tealHov : C.teal,
                      color: "#fff", border: "none", borderRadius: 6,
            ...
            {([
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: hovBtn === card.id ? "0 4px 16px rgba(14,124,123,0.15)" : "0 1px 4px rgba(0,0,0,0.08)",
                  border: `1px solid ${hovBtn === card.id ? C.teal : C.border}`,
                  display: "flex", flexDirection: "column", gap: 10,
                }}
              >
                <span className="material-symbols-rounded" style={{ fontSize: 32, color: card.color }}>{card.icon}</span>
                <div>
      ...
      {modal === "newRdv" && (
        <ModalOverlay onClose={() => setModal(null)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Date">
              <input type="date" style={inputStyle} value={newRdvForm.date} onChange={(e) => setNewRdvForm((f) => ({ ...f, date: e.target.value }))} />
            </Field>
            <Field label="Heure">
              <select style={inputStyle} value={newRdvForm.heure} onChange={(e) => setNewRdvForm((f) => ({ ...f, heure: e.target.value }))}>
      ...
      {modal === "newPatient" && (
        <ModalOverlay onClose={() => setModal(null)}>
          ...
          <Field label="Nom complet">
            <input style={inputStyle} value={newPatientForm.nom} onChange={(e) => setNewPatientForm((f) => ({ ...f, nom: e.target.value }))} placeholder="Prénom et nom" />
          </Field>
          <Field label="Téléphone">
            <input style={inputStyle} value={newPatientForm.telephone} onChange={(e) => setNewPatientForm((f) => ({ ...f, telephone: e.target.value }))} placeholder="+236 75 xx xx xx" />
          </Field>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Âge">
              <input type="number" style={inputStyle} value={newPatientForm.age} onChange={(e) => setNewPatientForm((f) => ({ ...f, age: e.target.value }))} placeholder="Ex: 34" min={0} max={120} />
            </Field>
            <Field label="Village">
              <input style={inputStyle} value={newPatientForm.village} onChange={(e) => setNewPatientForm((f) => ({ ...f, village: e.target.value }))} placeholder="Ex: Boyali" />
            </Field>
          </div>
